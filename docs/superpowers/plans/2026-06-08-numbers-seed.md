# seed-numbers.ts Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `scripts/seed-numbers.ts` that additively seeds the book of Numbers — tribal princes, Levitical clan heads, Korah's rebellion, Balaam, the Peor incident, and Zelophehad's daughters — without duplicating anyone already in the DB.

**Architecture:** Single TypeScript script following the `seed-exodus.ts` pattern. Key addition: `safeInsertPerson` checks the DB by name before inserting, so the script is safe to run regardless of what prior seeds (Leviticus, partial Numbers) have already added. Relationships to Genesis/Exodus people use `insertRelByName` (name-based DB lookup).

**Tech Stack:** TypeScript, tsx, @libsql/client (Turso), dotenv

---

### Task 1: Scaffold the file with boilerplate and `safeInsertPerson`

**Files:**
- Create: `scripts/seed-numbers.ts`

- [ ] **Step 1: Create the file with imports, DB client, and shared helpers**

Write `scripts/seed-numbers.ts`:
...
```bash
./node_modules/.bin/tsx --no-warnings scripts/seed-numbers.ts 2>&1 | head -5
```

Expected: either silent exit or a `process.exit` not-yet-defined error — NOT a TypeScript parse error. (The file has no `main()` yet so it will just exit cleanly.)

- [ ] **Step 3: Commit the scaffold**

```bash
git add scripts/seed-numbers.ts
git commit -m "feat: scaffold seed-numbers.ts with safeInsertPerson helper"
```

---

### Task 2: Implement `seedPeople()`

**Files:**
- Modify: `scripts/seed-numbers.ts`

Add the full `seedPeople()` function after the helpers. This covers all six groups from the spec.

- [ ] **Step 1: Add `seedPeople()` to the file**

Append to `scripts/seed-numbers.ts`:
...
```bash
git add scripts/seed-numbers.ts
git commit -m "feat: add seedPeople() to seed-numbers.ts — 28 people across 6 groups"
```

---

### Task 3: Implement `seedRelationships()`

**Files:**
- Modify: `scripts/seed-numbers.ts`

- [ ] **Step 1: Append `seedRelationships()` to the file**
...
async function seedRelationships() {
  ...
  await insertRelByName("Judah",    "ancestor_of", "Nahshon",   "Nahshon is prince of Judah (Num 1:7)");
  await insertRelByName("Issachar", "ancestor_of", "Nethanel",  "Nethanel is prince of Issachar (Num 1:8)");
  await insertRelByName("Zebulun",  "ancestor_of", "Eliab",     "Eliab son of Helon is prince of Zebulun (Num 1:9)");
  await insertRelByName("Reuben",   "ancestor_of", "Elizur",    "Elizur is prince of Reuben (Num 1:5)");
  await insertRelByName("Simeon",   "ancestor_of", "Shelumiel", "Shelumiel is prince of Simeon (Num 1:6)");
  await insertRelByName("Gad",      "ancestor_of", "Eliasaph",  "Eliasaph is prince of Gad (Num 1:14)");
  await insertRelByName("Ephraim",  "ancestor_of", "Elishama",  "Elishama is prince of Ephraim (Num 1:10)");
  await insertRelByName("Manasseh", "ancestor_of", "Gamaliel",  "Gamaliel is prince of Manasseh (Num 1:10)");
  await insertRelByName("Benjamin", "ancestor_of", "Abidan",    "Abidan is prince of Benjamin (Num 1:11)");
  await insertRelByName("Dan",      "ancestor_of", "Ahiezer",   "Ahiezer is prince of Dan (Num 1:12)");
  await insertRelByName("Asher",    "ancestor_of", "Pagiel",    "Pagiel is prince of Asher (Num 1:13)");
  await insertRelByName("Naphtali", "ancestor_of", "Ahira",     "Ahira is prince of Naphtali (Num 1:15)");

  // Nahshon ↔ Elisheba (sibling — Aaron's wife is Nahshon's sister)
  ...
  await insertRelByName("Manasseh", "ancestor_of", "Zelophehad", "Zelophehad is from the clan of Manasseh (Num 27:1)");
}
```
...
```bash
git add scripts/seed-numbers.ts
git commit -m "feat: add seedRelationships() to seed-numbers.ts"
```

---

### Task 4: Implement `seedRefs()`

**Files:**
- Modify: `scripts/seed-numbers.ts`

- [ ] **Step 1: Append `seedRefs()` to the file**

```typescript
async function seedRefs() {
  // ── Twelve princes — census appointment ───────────────────────────────
...
```bash
git add scripts/seed-numbers.ts
git commit -m "feat: add seedRefs() to seed-numbers.ts"
```

---

### Task 5: Add `main()`, wire `package.json`, run and verify

**Files:**
- Modify: `scripts/seed-numbers.ts`
- Modify: `package.json`

- [ ] **Step 1: Append `main()` to the file**
...
- [ ] **Step 2: Add `seed:numbers` to `package.json`**

In `package.json`, find:
```json
"seed:exodus": "npx tsx scripts/seed-exodus.ts"
```
Change to:
```json
"seed:exodus": "npx tsx scripts/seed-exodus.ts",
"seed:numbers": "npx tsx scripts/seed-numbers.ts"
```
...
Expected output (counts will vary based on what's already in DB):
```
Seeding Numbers people...
Seeding Numbers relationships...
Seeding Numbers scripture references...

✓ Numbers seed complete.
  Total people now: <number greater than before>
  Total relationships now: <number greater than before>
  Total scripture refs now: <number greater than before>
```

Any `⚠ Could not link` warnings are acceptable for cross-book links if the referenced person hasn't been seeded yet. Zero errors (non-warning lines going to stderr) is required.
...
Expected: same counts as the first run — no new rows added on re-run.

- [ ] **Step 5: Commit**

```bash
git add scripts/seed-numbers.ts package.json
git commit -m "feat: complete seed-numbers.ts — tribal princes, Levites, Korah, Balaam, Zelophehad"
```
