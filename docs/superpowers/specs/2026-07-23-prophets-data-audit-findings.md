...
> **Superseded — see Finding 11.** This finding's premise (that a pre-existing duplicate relationship already existed) turned out to be false; deleting the row it identified left zero relationships between Jonah and Jeroboam II. Finding 11 documents the discovery and the corrective restore.
...
**Note added during Task 3:** Finding 11 was appended later, during Task 3's live verification, after this original triple-check pass — it documents a regression discovered post-hoc (Finding 8's premise was found to be false; see Finding 11 and its superseded-banner note on Finding 8 above) and was not itself part of the "10 findings"/Step-5 read-through described above.
