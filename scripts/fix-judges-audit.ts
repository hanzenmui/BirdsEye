...
async function main() {
  ...
  const othnielId = await resolveExisting("othniel", "Othniel");

  // ───────────────────────────────────────────────────────────────────────
  // Finding 1: Othniel's relationship to Caleb — "brother" vs. "nephew" is a
  // genuine, long-standing textual ambiguity (Judg 1:13, 3:9; Josh 15:17)
  // that scripts/seed-judges.ts line 88's othniel.description states as
  // settled fact ("Caleb's younger brother"), while a comparably weighty
  // scholarly tradition (per Cambridge Bible for Schools and Colleges,
  // BibleRef.com, GotQuestions.org) holds Othniel was Caleb's nephew, with
  // Kenaz — not Othniel — as Caleb's actual brother. The controller has
  // decided on Option 1 (minimal, recommended) from the findings doc: keep
  // the existing `Caleb sibling_of Othniel` relationship as-is (it reflects
  // a real, traditionally-favored reading), but soften the description's
  // wording to acknowledge the alternate reading instead of stating it as
  // flatly settled. No relationship change — the sibling_of edge is
  // intentionally left untouched per the controller's decision.
  // ───────────────────────────────────────────────────────────────────────

  // Finding 1
  await run("Finding 1", `UPDATE people.description for othniel (id: ${othnielId})`, {
    sql: `UPDATE people SET description = ? WHERE id = ?`,
    args: [
      "Son of Kenaz. Traditionally identified as Caleb's younger brother (Judg 3:9; Josh 15:17), though the Hebrew syntax is ambiguous and some readings make him Caleb's nephew instead (with Kenaz as Caleb's brother). Captured Debir and received Achsah (Caleb's daughter) as his wife. Became the first judge of Israel after the conquest, delivering Israel from Cushan-rishathaim, king of Aram-Naharaim. He judged Israel forty years. A model judge: the Spirit of the Lord came upon him, he went to war, prevailed, and there was peace during his lifetime.",
      othnielId,
    ],
  });
