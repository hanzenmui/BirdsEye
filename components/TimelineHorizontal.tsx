...
function EventLane({ events, range, selectedEventId, onSelectEvent }: EventLaneProps) {
  ...
  // is a PIXEL problem, not a year problem (it depends on label width and zoom),
  // so it can only be settled after layout.
  //
  // Measure the LABEL, not the marker: `.tl-event` is deliberately zero-width so
  // the dot can centre exactly on its year, which means measuring the host
  // reports a point and nothing ever appears to collide — every label lands on
  // row 0 and they pile on top of each other.
  //
  // Events are greedily pushed onto lower rows, but only up to MAX_EVENT_ROWS.
  // Past that the lane would be a wall of text taller than the chart it
  // annotates, so any label that still will not fit is hidden and only its dot
  // remains — the event stays visible, hoverable and clickable, with its title
  // in the tooltip and the detail panel. A selected event always keeps its label.
  //
  ...
  useEffect(() => {
    ...
    for (const p of placed) {
      ...
      if (fitted) {
        ...
        // Never hide what the reader just clicked.
        p.el.style.top = "0px";
