...
interface PersonIndexCardProps {
  ...
  selected?: boolean;
  // Set when a chapter is selected and this person's reference only spans
  // through it rather than starting in it — real context, not this chapter's
  // own cast. Unused outside the chapter view.
  spanOnly?: boolean;
  onSelect: () => void;
}

function PersonIndexCard({ person, index, referenceCount, referenceLabels = [], selected = false, spanOnly = false, onSelect }: PersonIndexCardProps) {
  ...
  return (
    <button
      type="button"
      className={`person-card${selected ? " selected" : ""}${spanOnly ? " person-card-span" : ""}`}
...
function BooksSection({ people, refs, onSelect }: BooksSectionProps) {
  ...
  // Reading order, not alphabetical: within a book people are sorted by where
  // they first show up, so the list tracks the page you are on.
  //
  ...
  const bookPeople = useMemo(() => {
    ...
    return [...firstAt.entries()]
      ...
      .filter((e): e is { person: Person; spanOnly: boolean } => Boolean(e.person));
  }, [activeBook, activeChapter, bookRefs, peopleById]);

  // Only the references that justify the current view: on a chapter, the ones
  // that actually touch it, so a card does not advertise verses you filtered away.
  const activeBookRefs = useMemo(() => {
    const grouped = new Map<string, ScriptureRef[]>();
    if (!activeBook) return grouped;
    bookRefs.forEach(ref => {
      if (activeChapter !== null && !refCoversChapter(ref, activeChapter)) return;
          ...
          <>
            <div className="book-detail-header">
              ...
              <div className="book-detail-title-row">
                <h2>{activeBook}{activeChapter !== null ? ` ${activeChapter}` : ""}</h2>
